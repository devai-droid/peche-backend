...
export class BlogCommonTextController {
  ...
  @Get("public")
  publicActive(@Query("lang") lang?: string) {
    return this.service.findActive(lang)
  }

  @ApiOperation({ summary: "공통 고지문구 전체 (어드민 편집용 — 언어별 4종)" })
  @Get()
  @Auth(AuthGuard(JWT_STRATEGY), SWAGGER_TOKEN_NAME, Role.ADMIN)
  findAll(@Query("lang") lang?: string) {
    return this.service.findAllForAdmin(lang)
  }

  @ApiOperation({ summary: "공통 고지문구 수정 (lang+type별 upsert)" })
  @Put(":type")
  @Auth(AuthGuard(JWT_STRATEGY), SWAGGER_TOKEN_NAME, Role.ADMIN)
  upsert(
    ...
    return this.service.upsert(lang || "ko", type, dto, user)
  }
}
