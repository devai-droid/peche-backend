...
export class BlogV2PostService {
  ...
  async findMany(query: QueryBlogPostDto): Promise<PaginatedResult<BlogPostV2>> {
    ...
    if (query.productCategoryId) {
      ...
      // 외국어 글은 product_page가 그 언어 상품명(en=Ultherapy Prime)이라, 한국어 dp.name 외에 해당 언어 이름 컬럼도 매칭.
      const catNameCol = BlogV2PostService.detailNameCol((query.lang as string) ?? "ko")
      qb.andWhere(
        `(p.product_category_id = :pid OR EXISTS (
            SELECT 1 FROM public.product_detail_page dp
            WHERE dp.category_id = :pid
              AND EXISTS (SELECT 1 FROM unnest(string_to_array(p.product_page, '|')) AS e WHERE trim(e) = dp.name OR trim(e) = dp.${catNameCol})
          ))`,
