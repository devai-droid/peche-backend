...
export class BlogImageUploadService {
  ...
  /** S3 업로드. 로컬 환경(IS_LOCAL_ENV)에서 S3 실패 시 로컬 디스크 저장 + 정적 URL fallback. */
  private async uploadOne(f: Express.Multer.File, name: string): Promise<string> {
    try {
      return (await this.uploadService.uploadImage(f)).url
    } catch (err) {
      if (process.env.IS_LOCAL_ENV === "1") {
        this.logger.warn(`S3 실패 → 로컬 저장 fallback (${name})`)
        return this.saveLocal(f, name)
      }
      throw err
    }
  }

  private saveLocal(f: Express.Multer.File, name: string): string {
