...
export function createPaginationObject<T, CustomMetaType extends ObjectLiteral = IPaginationMeta>({
  ...
  const routes: IPaginationLinks =
    totalItems !== undefined
      ? {
          ...
          previous: hasPreviousPage ? `${route}${symbol}${pageLabel}=${currentPage - 1}&${limitLabel}=${limit}` : "",
          next: hasNextPage ? `${route}${symbol}${pageLabel}=${currentPage + 1}&${limitLabel}=${limit}` : "",
  ...
  return new Pagination<T, CustomMetaType>(items, meta as any, links)
}
